...
- **Custom step types** (`process-mapper/settings/`): a Settings page for managing the block-type palette — each type is a name, a shape picked from 12 built-in geometries (rectangle, rounded, pill, circle, diamond, parallelogram, trapezoid, hexagon, document, cylinder, cloud, subroutine) and a colour. Ships with four built-in types (Process, Decision, Terminal, Document) that are protected from deletion; custom types can be added, edited, reordered and deactivated. Stored in the `process_step_types` lookup table; the four API endpoints (`get` / `save` / `delete` / `reorder_step_types.php`) mirror the Tasks-module lookup pattern.
